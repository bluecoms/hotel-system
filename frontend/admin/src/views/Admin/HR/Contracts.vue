...
# Version   : 2025.11-10 · v4.8 (Final Stable · Full Merge Edition)
# Purpose   : Hotel Admin — HR 직원 계약 관리 (미계약자 포함 + 최신 계약만 표시)
# ----------------------------------------------------------------------------
# 주요 특징
#   • 직원(Employee) 기준 계약 목록/이력/종료/스캔본 업로드 관리
#   • 미계약자도 목록에 표시 (Employee + Contract merge)
#   • 동일 직원은 최신 계약(version_no 최대 또는 is_latest)만 표시
#   • 급여 소수점 제거 (₩#,###)
...
<template>
  <v-container fluid class="page-shell py-6">
    ...
    <div class="bar mb-6">
      ...
      <div class="bar-right">
        <!-- 신규 계약 버튼 -->
        <v-btn color="primary" prepend-icon="mdi-file-document-plus"
               variant="flat" class="btn-action" @click="openNewContract">
          신규 계약
        </v-btn>
      </div>
    </div>
    ...
    <BoardList title="계약 목록"
      ...
      <template #cell.actions="{ item }">
        ...
        <v-tooltip text="스캔본 업로드 (확정)">
          <template #activator="{ props }">
            <v-btn v-bind="props" icon="mdi-file-upload-outline"
        ...
        <v-tooltip text="계약 종료">
          <template #activator="{ props }">
            <v-btn v-bind="props" icon="mdi-file-cog-outline"
    ...
    <v-dialog v-model="uploadDlg.open" max-width="420">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field v-model="uploadDlg.start_date" type="date"
                        label="계약 시작일" density="comfortable"
                        hide-details class="mb-2" />
          <v-text-field v-model="uploadDlg.end_date" type="date"
                        label="계약 종료일(선택)" density="comfortable"
                        hide-details />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
...
/* ============================================================================
# Script Summary
#   • 직원 목록 + 계약 목록 병합 (미계약자 표시)
#   • 동일 직원 최신 계약만 남김 (is_latest 또는 version_no 기준)
#   • 급여 정수 포맷 / 상태 컬러 / 토스트 통합
# ============================================================================ */
...
/* ───────────── 포맷터 ───────────── */
function fmtNum(n:number){return Number(n ?? 0).toLocaleString('ko-KR')}
...
async function reload(){
  ...
  try {
    ...
    for (const c of contracts) {
      ...
      if (!latestMap[empId] || (c.version_no ?? 0) > (latestMap[empId].version_no ?? 0)) {
        latestMap[empId] = c
      }
    }
    ...
    rows.value = employees.map(emp => {
      const c = latestMap[emp.id]
      return c
        ? { ...c, emp_name: emp.name, emp_no: emp.emp_no, status: c.status || 'active' }
...
async function onFilePicked(e:Event){
  ...
  }finally{uploadDlg.loading=false}
}
