  ...
  >
    ...
    <v-card v-else>
      ...
      <v-table density="comfortable">
        ...
        <tbody>
          <tr v-for="r in rows" :key="r.expense_type">
...
const rows = ref<{ expense_type: string; count: number; amount: number }[]>([])
...
async function onFilter(p: { date_from: string; date_to: string; property_code: string }) {
  ...
  try {
    const url = `/api/reports/expenses?property_code=${propertyCode.value}&date_from=${dateFrom.value}&date_to=${dateTo.value}`
