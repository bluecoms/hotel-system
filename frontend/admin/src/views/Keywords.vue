        ...
        <v-card class="mb-4">
          <v-card-text>
            <div class="d-flex flex-wrap align-center" style="gap:12px">
              ...
              <v-spacer />
              <v-btn variant="tonal" @click="exportCsv" :disabled="!rows.length">Export CSV</v-btn>
        ...
        <v-table density="comfortable">
          ...
          <tbody>
            <tr v-for="r in rows" :key="r.id">
              ...
              <td>
                <v-chip :color="r.is_active ? 'green' : 'grey'" size="small" label>
                  {{ r.is_active ? 'Y' : 'N' }}
                </v-chip>
              </td>
              <td class="d-flex" style="gap:8px">
                <v-btn size="small" variant="text" @click="openEdit(r)">Edit</v-btn>
                <v-btn size="small" variant="tonal" color="red" @click="remove(r)">Delete</v-btn>
              </td>
            </tr>
            <tr v-if="!rows.length">
        ...
        <v-dialog v-model="dlgImport" max-width="640">
          <v-card>
            ...
            <v-card-text>
              ...
              <input type="file" ref="importRef" accept=".csv,.xlsx,.xls,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel" />
              <v-alert v-if="importMsg" type="info" class="mt-3">{{ importMsg }}</v-alert>
              <v-alert v-if="importErr" type="warning" class="mt-3">{{ importErr }}</v-alert>
            </v-card-text>
            <v-card-actions>
              <v-spacer/>
        ...
        <v-card>
          ...
          <v-card-text>
            ...
            <div class="d-flex flex-wrap" style="gap:8px">
              <v-chip
                v-for="t in result.tags" :key="t.code"
                color="blue"
                label
              >
            ...
            <v-alert v-if="testErr" type="warning" class="mt-3">{{ testErr }}</v-alert>
          </v-card-text>
        </v-card>
      </v-window-item>
    </v-window>
...
const groupOptions = [
  { title: 'sales.tag.alias', value: 'sales.tag.alias' },         // 패턴 → 표준 태그 (O|바다 -> SEA 같은 용도)
  // 필요 시 카테고리/보조 그룹을 더 추가해 확장 가능
]
...
async function load(p = page.value) {
  ...
  try{
    ...
    err.value = e?.detail ?? e?.message ?? 'Load failed'
  }
}
...
async function save(){
  try{
    ...
    err.value = e?.detail ?? e?.message ?? 'Save failed'
  }
}

/* Delete */
async function remove(r: KeywordRow){
  if (!confirm(`#${r.id} 삭제할까요?`)) return
  try{
    await http.delete(`keywords/${r.id}`)
    await load(page.value)
  }catch(e:any){
    err.value = e?.detail ?? e?.message ?? 'Delete failed'
  }
}
...
async function exportCsv(){
  try{
    ...
    while(true){
      ...
      items.push(...(r.items||[]))
      const lastPage = Math.max(1, Math.ceil((r.total||0) / (r.size||1)))
      if (p >= lastPage) break
      p++
    }
    ...
    err.value = e?.detail ?? e?.message ?? 'Export failed'
  }
}
...
function openImport(){ importMsg.value=null; importErr.value=null; if(importRef.value) importRef.value.value=''; dlgImport.value = true }
...
// CSV 라인 split(간단 처리: 따옴표 내부 콤마 보존)
...
// 헤더 매핑 또는 기본 순서(k,v,weight,is_active)
...
const useRegexWordBoundary = ref(true)   // 토큰 매칭 시 경계 사용
...
async function reloadRules(){
  ...
  try{
    ...
    const r = await http.get<{items:KeywordRow[]}>(
      `keywords?${params.toString()}`
    )
    rules.value = r.items || []
  }catch(e:any){
    testErr.value = e?.detail ?? e?.message ?? 'Rules load failed'
    rules.value = []
  }
}
