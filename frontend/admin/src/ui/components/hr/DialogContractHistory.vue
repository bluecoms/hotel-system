<!-- ============================================================================
  Hotel Admin — 계약 이력 다이얼로그 (DialogContractHistory.vue)
  ----------------------------------------------------------------------------
  Version : 4.0.0 | Date : 2025-10-18 | Owner : Hotel Admin (HR)
  목적:
    • 근로계약의 변경·만료 이력을 직관적으로 표시하는 다이얼로그
    • 버전별 계약기간, 급여, 상태를 보기 좋게 정렬
    • 목록 없을 때는 StateBlock 컴포넌트로 공백상태 안내
  변경이력:
    - 4.0.0 : UI 전면 개선 (v-data-table 기반, Chip 컬러 통일)
    - 3.2.0 : 버전·상태 컬럼 라벨 보강, 한글화 완료
============================================================================ -->
<template>
  <v-dialog
    :model-value="open"
    ...
    <v-card class="rounded-xl">
      ...
      <v-card-text class="px-5 py-4">
        ...
        >
          ...
          <!-- 급여 -->
          <template #item.salary_monthly="{ item }">
            <div class="text-right font-weight-medium">
              {{ fmtCurrency(item.salary_monthly) }}
            </div>
          </template>
...
/** ─────────────── Props / Emits ─────────────── */
const props = defineProps<{ open: boolean; contractId: string | number }>()
...
const headers = [
  ...
  { title: '월급(세전)', key: 'salary_monthly', align: 'end', width: '140px' },
