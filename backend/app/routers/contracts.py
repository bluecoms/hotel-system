...
# ----------------------------------------------------------------------------
# 목적:
#   • 직원(Employee)별 계약 생성/조회/이력/확정 관리
...
def list_contracts(
    ...
    q: Optional[str] = Query(None, description="검색어 (이름/부서/직책 등)"),
    status: Optional[str] = Query(None, description="계약 상태 필터"),
    ...
    --------------------------------------------------------------------------
    - LEFT JOIN:
        Employee ← EmployeeContract (직원기준)
        Employee.dept_code → MasterDepartment.dept_code (부서명 조회)
    - 계약이 없는 직원도 "미계약" 상태로 표시
    ...
    qset = (
        ...
        .outerjoin(
            MasterDepartment,
            and_(
                Employee.dept_code == MasterDepartment.dept_code,
    ...
    if status:
        if status == "none":
            ...
            qset = qset.filter(EmployeeContract.status == status)
