...
# Version : 2025-10-23 · v3.1 (Optional Password · SSOT Stable)
...
# ----------------------------------------------------------------------------
# 변경사항 (v3.1)
#   ✅ UserCreate.password 필드 추가 (Optional)
#   ✅ UserListOut → employee_id 필드 유지
...
class UserCreate(BaseModel):
    ...
    password: Optional[str] = Field(None, description="초기 비밀번호 (선택, 없으면 기본 hotel1234)")
    is_active: bool = Field(True, description="활성 여부 (기본값: True)")
...
class UserListOut(BaseModel):
    ...
    email: EmailStr
    is_active: bool
    employee_id: Optional[int] = Field(None, description="연결된 사원 ID (없으면 None)")
...
class UserActivateIn(BaseModel):
    """사용자 활성화/비활성화 입력"""
    is_active: bool = Field(True, description="활성 여부 (True=활성, False=비활성)")
...
class CreateFromEmployeeIn(BaseModel):
    ...
    is_active: bool = Field(True, description="활성 여부 (기본 True)")
