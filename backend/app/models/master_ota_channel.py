# -*- coding: utf-8 -*-
# ============================================================================
# File      : app/models/master_ota_channel.py
# Version   : 2025.10-25 · v1.0 (Initial · SSOT Standard)
# Purpose   : Hotel Admin — Master OTA Channel Model
# ----------------------------------------------------------------------------
# 목적:
#   • OTA 채널(Booking.com, Agoda, Expedia 등) 기준정보의 단일 소스(SSOT)
#   • 커미션(수수료) 및 OTA 주문/요약 등에서 참조되는 기준 테이블
# ----------------------------------------------------------------------------
# 필드:
#   - id         : PK
#   - code       : 채널 코드(고유, 대문자 권장)  ex) BOOKING, AGODA, EXPEDIA, NAVER, AIRBNB
#   - name       : 채널 표시명                     ex) "Booking.com"
#   - is_active  : 사용 여부(기본 True)
#   - order_no   : 정렬용(선택)
#   - created_at : 생성 시각
#   - updated_at : 수정 시각
# ----------------------------------------------------------------------------
# 참고:
#   • 실제 테이블 생성은 Alembic 마이그레이션에서 처리합니다(모델만 정의).
#   • 스키마(Pydantic)와 라우터는 별도 파일에 정의/연결합니다.
# ============================================================================

from __future__ import annotations

from typing import Optional

from sqlalchemy import Column, Integer, String, Boolean, DateTime, func, UniqueConstraint, Index
from app.db.base_class import Base


class MasterOtaChannel(Base):
    """
    OTA 채널 기준정보(SSOT)
    - code 는 고유(unique)하며 대문자 사용 권장
    - name 은 표시용 이름
    """
    __tablename__ = "master_ota_channels"

    id: int = Column(Integer, primary_key=True, index=True, autoincrement=True)

    # ex) "BOOKING", "AGODA", "EXPEDIA", "NAVER", "AIRBNB"
    code: str = Column(String(50), nullable=False, unique=True, index=True)

    # ex) "Booking.com"
    name: str = Column(String(100), nullable=False)

    is_active: bool = Column(Boolean, nullable=False, server_default="1", default=True)

    # 선택: 리스트 정렬/표시 우선순위
    order_no: Optional[int] = Column(Integer, nullable=True)

    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=True, onupdate=func.now())

    __table_args__ = (
        # code 고유성 보강(대문자 사용 권장, 애플리케이션 레벨에서도 검증)
        UniqueConstraint("code", name="uq_master_ota_channels_code"),
        Index("ix_master_ota_channels_active_order", "is_active", "order_no"),
    )

    # ─────────────────────────────────────────
    # 표현/유틸
    # ─────────────────────────────────────────
    def __repr__(self) -> str:
        return f"<MasterOtaChannel id={self.id} code={self.code!r} name={self.name!r} active={self.is_active}>"
